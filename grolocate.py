...
def lookup_phone_location(phone_number):
    ...
    try:
        ...
        if response.status_code == 200 and data.get('code') == 0:
            ...
            return f"{phone_number} - 查询失败"
            
    except Exception as e:  # 处理网络请求或JSON解析异常
        return f"{phone_number} - API请求异常: {str(e)}"
...
# 添加线索处理函数
def add_custom_clue(manual_input, current_clues):
    """添加自定义线索到列表"""
    if manual_input.strip() and manual_input not in current_clues:
        new_clues = current_clues + [manual_input]
        choices = [f"{clue} (手动添加)" for clue in new_clues]
        return new_clues, choices
    return current_clues, gr.update()
...
def lookup_plate_location(plate_number):
    ...
    "青":"青海省",}
    output = plate_number
    if plate_number[0] in cardic.keys():
        output += f" - {cardic[plate_number[0]]}"
        if plate_number[0:2] in cardic.keys():
            output += f" - {cardic[plate_number[0:2]]}"
    else:
        output += "暂无归属地信息"
